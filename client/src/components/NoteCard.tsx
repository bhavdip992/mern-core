    ...
    return (
        <div className={`bg-white p-4 rounded-lg shadow border-0 hover:shadow-gray-100 hover:shadow-2xl hover:border-2 ${note.completed ? 'opacity-50' : ''}`}  >
            <div className="flex justify-between">
                ...
                {!note.completed ? <button className={`completed-todos cursor-pointer hover:bg-gray-100 p-1 rounded-2xl ${note.completed ? 'pointer-events-none' : ''}`} onClick={onComplete}>
            ...
            <div className="flex justify-between items-center mt-3">
                ...
                <div className="flex gap-2">
                    {!note.completed ? <button onClick={onEdit} className={`cursor-pointer hover:bg-gray-100 p-2 rounded-2xl ${note.completed ? 'pointer-events-none' : ''}`}>✏️</button> : ''}
